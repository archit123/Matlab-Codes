...
if nargin <= 1,
    ...
    return
elseif nargin == 2,
    ltol = 0.001; maxiter = 1000; pflag = 0; Init = [];
    err_X = Verify_X(X);
    err_k = Verify_k(k);
...
[E,Ln] = Expectation(X,k,W,M,V); % Initialize log likelihood
Lo = 2*Ln;
...
while (abs(100*(Ln-Lo)/Lo)>ltol) & (niter<=maxiter),
    ...
    Lo = Ln;
    [E,Ln] = Expectation(X,k,W,M,V);
    niter = niter + 1;
    LnValues(niter) = Ln; 
     niter_vector(niter)=niter;
     Ln_vector( niter)=Ln;
